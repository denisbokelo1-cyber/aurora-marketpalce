...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body {{ $is_rtl == 1 ? 'dir=rtl' : '' }}>
        ...
        @if (!file_exists($sqlDumpPath) && !file_exists($installViewPath))
            @php
                $currency_code = session('currency') ?? ($system_settings['currency_setting']['code'] ?? 'USD');
                $currency_details = fetchDetails(Currency::class, ['code' => $currency_code]);
                $currency_details_arr = $currency_details ?? [];
                $currency_symbol = !empty($currency_details_arr) ? ($currency_details_arr[0]->symbol ?? '$') : '$';
            @endphp
