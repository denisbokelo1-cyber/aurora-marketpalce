                ...
                <form id="translation_bulk_upload_form" action="{{ route('admin.translation_bulk_upload') }}" method="POST"
                    ...
                    <div class="aurora-card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="aurora-input" name="type" id="type" >
                                <option value="">{{ labels('admin_labels.select', 'Sélectionner') }}</option>
                                <option value="brands">{{ labels('admin_labels.brands_option', 'Marques') }}</option>
                                <option value="categories">{{ labels('admin_labels.categories_option', 'Catégories') }}</option>
                                <option value="cities">{{ labels('admin_labels.cities_option', 'Cities') }}</option>
                                <option value="stores">{{ labels('admin_labels.stores_option', 'Boutiques') }}</option>
                                <option value="taxes">{{ labels('admin_labels.taxes_option', 'Taxes') }}</option>
                                <option value="products">{{ labels('admin_labels.products_option', 'Produits') }}</option>
                                <option value="combo_products">{{ labels('admin_labels.combo_products_option', 'Combo Products') }}</option>
                                <option value="offers">{{ labels('admin_labels.offers_option', 'Offres') }}</option>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary submit_button">
                                {{ labels('admin_labels.submit', 'Enregistrer') }}
                            </button>
                        </div>
