...
class OffersSection extends Component
{
    public function render()
    {
        ...
        // Fallback if no store in session
        if (empty($store_id)) {
            $defaultStore = \App\Models\Store::where('is_default_store', 1)->orWhere('status', 1)->first();
    ...
    public function getOffers($store_id)
    {
        ...
        foreach ($offers as $key => $offer) {
            // Keep remote URLs as-is, only process local files through media service
            $image = !empty($offer->image)
                ? (filter_var($offer->image, FILTER_VALIDATE_URL)
                    ? $offer->image
                    : (file_exists(public_path(config('constants.MEDIA_PATH') . $offer->image))
                        ? app(MediaService::class)->getImageUrl($offer->image)
                        : ''))
                : '';
            $banner_image = !empty($offer->banner_image)
                ? (filter_var($offer->banner_image, FILTER_VALIDATE_URL)
                    ? $offer->banner_image
