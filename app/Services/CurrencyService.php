    ...
    public function currentCurrencyPrice($price, $with_symbol = false)
    {
        ...
        if ($resolved === null) {
            ...
            if (isset($shipping_settings['shiprocket_shipping_method']) && $shipping_settings['shiprocket_shipping_method'] == 1) {
                ...
                $currency_setting = $system_settings['currency_setting'] ?? [];
                $currency_code = session('currency') ?? ($currency_setting['code'] ?? 'USD');
                $currency_details = $this->getCurrencyCodeSettings($currency_code);
                $currency_details_row = $currency_details[0] ?? [];
                $resolved = [
                    'symbol' => $currency_details_row['symbol'] ?? ($system_settings['currency_setting']['symbol'] ?? '$'),
                    'rate'   => (float) ($currency_details_row['exchange_rate'] ?? 1),
                ];
            }
        }
